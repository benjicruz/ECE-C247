...
def affine_forward(x, w, b):
  ...
  #   assignments.
  # ================================================================ #
  x_ND = x.reshape(x.shape[0], -1) # NxD
  out = x_ND.dot(w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_i = (x - minibatch_mean)/(np.sqrt(minibatch_var + eps))
    out = gamma * x_i + beta
    cache = (minibatch_mean, minibatch_var, x_i, x, beta, gamma, eps)
    ...
    # ================================================================ #
    x_i_hat = (x - running_mean) / np.sqrt(running_var.T)
    out = gamma * x_i_hat + beta
...
def batchnorm_backward(dout, cache):
  ...
  minibatch_mean = cache[0]
  minibatch_var = cache[1]
  x_i = cache[2]
  x = cache[3]
  beta = cache[4] # never used
  gamma = cache[5]
  eps = cache[6]
  #begin
  #dbeta = sum of dout (dl/dy_j)
  dbeta = np.sum(dout, axis=0)
  #dgamma = sum of dl/dy * xhat
  dgamma = np.sum(dout * x_i, axis = 0) 
  #dx = gradient from xhat + gradient from variance + gradient from mean
  #           dx1               dx2                       dx3
  dxhat = gamma * dout # dl/dy * gamma
  da = (1 / np.sqrt(minibatch_var + eps)) * dxhat # 1/sqrt(var + e) * dxhat
  dx1 = da
  #### done ######
  dmu = -np.sum(da, axis=0) 
  dx3 = (dmu / dout.shape[0]) # dout shape 0 is M
  ##### done #####
  de = -0.5 / (np.sqrt(minibatch_var + eps)**3) * (x - minibatch_mean) * dxhat
  dsig =np.sum(de, axis=0)
  dx2 = (2 * (x - minibatch_mean) / dout.shape[0]) * dsig 
  #### done #####
  dx = dx1 + dx2 + dx3
