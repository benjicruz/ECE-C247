...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    exp = np.exp(scores)
    term = exp / (np.sum(exp, axis=1, keepdims=True))
    loss = -np.log(term[np.arange(N), y])
    loss = np.sum(loss) / N
    ...
    back1 = term
    back1[np.arange(N), y] -= 1
    back1 = back1 / N
    grads['W2'] = (np.dot(z1.T, back1)).T + reg * W2
    grads['b2'] = np.sum(back1, axis=0)
    DLDH2 = np.dot(back1, W2)
    DLDH2[z1 <= 0] = 0
    grads['W1'] = np.dot(DLDH2.T, X) + reg * W1
    grads['b1'] = np.sum(DLDH2, axis=0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # ================================================================ #
      i = np.random.choice(np.arange(num_train), batch_size)
      X_batch = X[i]
      y_batch = y[i]
